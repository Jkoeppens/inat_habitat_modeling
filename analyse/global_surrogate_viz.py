...
"""
Global Surrogate Tree Visualization

Pipeline:
  1) global_surrogate_train.py  -> surrogate_tree.json
  2) global_surrogate_viz.py    -> surrogate_tree.html

Features:
- Suitability-basiertes Layout (Leaves links→rechts nach suit)
- Entscheidungsboxen mit Farbverlauf und Threshold-Linie
- Suitability-Skala mit Dreiecken + Zahlen
- Leaf-Pfade: starten unten rechts/links der Threshold,
  laufen über die Mitte der Boxen und enden auf der Skala
- Interaktives Highlighting:
    * Hover über Node: alle Pfade durch diese Node werden hervorgehoben
    * Hover über Leaf-Dreieck: der Pfad dieses Leaves wird hervorgehoben
"""
...
def export_html(tree_json_path, out_html_path):
    data = json.loads(Path(tree_json_path).read_text(encoding="utf-8"))
    html = HTML.replace("TREE_JSON", json.dumps(data, ensure_ascii=False))
    Path(out_html_path).write_text(html, encoding="utf-8")
    print(f"✓ HTML exportiert nach: {out_html_path}")


def main():
    ap = argparse.ArgumentParser()
    ap.add_argument("--json", required=True)
    ap.add_argument("--out", default="surrogate_tree.html")
    args = ap.parse_args()
    export_html(args.json, args.out)
